<?php
    ...
    if(!isset($_SESSION['hotel_order']['room'])){exit('<h1>404</h1>');}
...
<!-- Main Wrapper Start -->
    <div class="home_black_version">
        ...
        <?php 
            $product_meta = $room->FindById($_SESSION['hotel_order']['room']);
            if(isset($_GET['pay'])){
                try{
                    $amount = intval(str_replace(',','', $_SESSION['hotel_order']['amt']));
                    $price = str_replace(',','',$util->Forex($product_meta['RoomPrice']))*$_SESSION['hotel_order']['n'];
                    ...
                    $narration = 'Hotel Room Booking via '. $settings['SiteName'];

                    if(!$roomorder->exists($_SESSION['hotel_order']['order'])){
                        /** create order if not exists*/
                        $o = new RoomOrder(
                            $conn, 
                            $_SESSION['usr']['UserId'],
                            $_SESSION['hotel_order']['order'],
                            $_SESSION['hotel_order']['room'], 
                            $_SESSION['hotel_order']['aqty'], 
                            $_SESSION['hotel_order']['cqty'], 
                            $_SESSION['hotel_order']['in'], 
                            $_SESSION['hotel_order']['out'], 
                            $_SESSION['hotel_order']['n'], 
                            $amount, 
                            $comm_amt
                        );
                        // $util->Show($_SESSION['hotel_order']);
                        $o->Create();
                    }
                    $amount = TEST_AMT;
                    $postfields = [
            ...
            <div class="container">
                <div class="checkout_form">
                    <?php if(isset($_SESSION['usr'])){?>
                        ...
                        <?php
                            $hourdiff = round((strtotime($_SESSION['hotel_order']['out']) - strtotime($_SESSION['hotel_order']['in']))/3600, 1);
                            $nights = floor($hourdiff/24);
                            $order_totals = $util->Forex($util->ApplyDiscountHotel($product_meta)*$nights);
                            $_SESSION['hotel_order']['n'] = $nights;
                            $_SESSION['hotel_order']['amt'] = $order_totals;
                        ?>
                            <form action="#"> 
                                ...
                                <div class="order_table table-responsive">
                                    <table>
                                        <tbody>
                                            <tr>
                                                <td> Room Name/Type <strong> </strong></td>
                                                <td><?= $product_meta['RoomName']?></td>
                                            </tr>
                                            <tr>
                                                <td> Adults Checking In</td>
                                                <td> <?= $_SESSION['hotel_order']['aqty']?></td>
                                            </tr>
                                            <tr>
                                                <td>  Children(under 4) Checking In	</td>
                                                <td> <?= $_SESSION['hotel_order']['cqty']?></td>
                                            </tr>
                                            <tr>
                                                <td> Checkin Date</td>
                                                <td> <?= $_SESSION['hotel_order']['in']?></td>
                                            </tr>
                                            <tr>
                                                <td> Checkout Date</td>
                                                <td> <?= $_SESSION['hotel_order']['out']?></td>
                                            </tr>
                                            <tr>
                                                <td> No. of Nights </td>
                                                <td> <?=$nights?></td>
                                            ...
                                            <tr>
                                                ...
                                                <td><strong><?=$_SESSION['cry'] .' ' .$util->Forex($util->ApplyDiscountHotel($product_meta))?></strong></td>
                                            </tr>
                                        </tbody>
                                        <tfoot>
