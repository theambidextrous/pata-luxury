                    ...
                    <div class="col-lg-4 col-md-6 col-sm-7">
                        <div class="widgets_container product_widget">
                            ...
                            <div class="simple_product">
                                <?php 
                                    $top_rated = $package->FindTopRated(2);
                                    foreach( $top_rated as $tprtd ):
                                        if($package->isReady($tprtd['PackageId'])){
                                ?>
                                <div class="simple_product_items">
                                    <div class="simple_product_thumb">
                                        <a href="<?=APP_HOME?>/package.php?item=<?=$tprtd['PackageId']?>"><img src="<?=APP_IMG_PATH?>items/<?=$gallery->FindByTypeProduct($tprtd['PackageId'],'5003')['GalleryPath']?>" alt=""></a>
                                    </div>
                                    <div class="simple_product_content">
                                        <div class="tag_cate">
                                            <a style="font-size: 11px;font-weight: 100;" href="package.php?htl=<?=$tprtd['PackageOwner']?>"><?=$user->FindById($tprtd['PackageOwner'])['UserFullName']?></a>
                                        </div>
                                        <div class="product_name">
                                            <h3><a href="<?=APP_HOME?>/package.php?item=<?=$tprtd['PackageId']?>"><?=$tprtd['PackageName']?></a></h3>
                                        </div>
                                        <div class="product_price">
                                            <?php if($tprtd['PackageDiscountValue'] > 0){?>
                                                <span class="old_price"><?=$_SESSION['cry'] .' ' .$util->Forex($tprtd['PackagePrice'])?></span>
                                            <?php } ?>
                                            <span class="current_price"><?=$_SESSION['cry'] .' ' .$util->Forex($util->ApplyDiscountHoliday($tprtd))?></span>
                                        </div>
                                    </div>
                                </div>
                                <?php 
