<?php
/**
 * @filename: BladeSMS.php
 * @role: SMS object
 * @author: avatar
 * @license : Proriatery
 */

class BytebladeSMSException extends Exception  {}
...
class BytebladeSMS
{
  ...
  public function sendMessage($to_, $message_, $from_ = null, $bulkSMSMode_ = 1, Array $options_ = array())
  {
    if ( strlen($to_) == 0 || strlen($message_) == 0 ) {
      throw new BytebladeSMSException('Please supply both to and message parameters');
    }
	...
	if ( in_array($key, $allowedKeys) && strlen($value) > 0 ) {
	  ...
	  throw new BytebladeSMSException("Invalid key in options array: [$key]");
	}
      }
    }
	...
	return $responseObject->SMSMessageData->Recipients;
      ...
      throw new BytebladeSMSException($responseObject->SMSMessageData->Message);
    }
    
    throw new BytebladeSMSException($this->_responseBody);
  }
  ...
  public function createSubscription($phoneNumber_, $shortCode_, $keyword_, $checkoutToken_)
  {
    ...
    if ( strlen($phoneNumber_) == 0 || strlen($shortCode_) == 0 || strlen($keyword_) == 0 || strlen($checkoutToken_) == 0 ) {
      throw new BytebladeSMSException('Please supply phoneNumber, shortCode, keyword and checkoutToken');
    }
  ...
  public function deleteSubscription($phoneNumber_, $shortCode_, $keyword_)
  {
    if ( strlen($phoneNumber_) == 0 || strlen($shortCode_) == 0 || strlen($keyword_) == 0 ) {
      throw new BytebladeSMSException('Please supply phoneNumber, shortCode and keyword');
    }
  ...
  public function call($from_, $to_)
  {
    if ( strlen($from_) == 0 || strlen($to_) == 0 ) {
      throw new BytebladeSMSException('Please supply both from and to parameters');
    }
	...
	return $responseObject->entries;
      ...
      throw new BytebladeSMSException($responseObject->errorMessage);
    }
    else
      throw new BytebladeSMSException($this->_responseBody);
  }
  ...
  public function bankPaymentCheckoutCharge($productName_,
    ...
    if($this->_responseInfo['http_code'] == self::HTTP_CODE_CREATED) {
      ...
      else throw new BytebladeSMSException($response->description);
    }
    throw new BytebladeSMSException($this->_responseBody);
  }
  ...
  public function mobilePaymentB2CRequest($productName_,
    ...
    if($this->_responseInfo['http_code'] == self::HTTP_CODE_CREATED) {
      ...
      else throw new BytebladeSMSException($response->errorMessage);
    }
    throw new BytebladeSMSException($this->_responseBody);
  }
  ...
  public function mobilePaymentB2BRequest($productName_, $providerData_, $currencyCode_, $amount_, $metadata_) {
		if(!isset($providerData_['provider']) || strlen($providerData_['provider']) == 0)
			throw new BytebladeSMSException("Missing field provider");
		
		if(!isset($providerData_['destinationChannel']) || strlen($providerData_['destinationChannel']) == 0)
			throw new BytebladeSMSException("Missing field destinationChannel");

    if(!isset($providerData_['destinationAccount']) || strlen($providerData_['destinationAccount']) == 0)
      throw new BytebladeSMSException("Missing field destinationAccount");
		
		if(!isset($providerData_['transferType']) || strlen($providerData_['transferType']) == 0)
			throw new BytebladeSMSException("Missing field transferType");
