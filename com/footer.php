                    ...
                    <div class="col-lg-4 col-md-6 col-sm-7">
                        <div class="widgets_container product_widget">
                            ...
                            <div class="simple_product">
                                <?php 
                                    $top_rated = $product->FindTopRated();
                                    foreach( $top_rated as $tprtd ):
                                        if($product->isReady($tprtd['ProductId'])){
                                ?>
                                <div class="simple_product_items">
                                    <div class="simple_product_thumb">
                                        <a href="<?=APP_HOME?>/product.php?item=<?=$tprtd['ProductId']?>"><img src="<?=APP_IMG_PATH?>items/<?=$gallery->FindByTypeProduct($tprtd['ProductId'],'5003')['GalleryPath']?>" alt=""></a>
                                    </div>
                                    <div class="simple_product_content">
                                            <div class="tag_cate">
                                            <?=$util->CategoryLinks($category, $tprtd['ProductCategories'])?>
                                        </div>
                                        <div class="product_name">
                                            <h3><a href="<?=APP_HOME?>/product.php?item=<?=$tprtd['ProductId']?>"><?=$tprtd['ProductName']?></a></h3>
                                        </div>
                                        <div class="product_price">
                                            <?php if($tprtd['ProductDiscountValue'] > 0){?>
                                                <span class="old_price"><?=$_SESSION['cry'] .' ' .$util->Forex($tprtd['ProductPrice'])?></span>
                                            <?php } ?>
                                            <span class="current_price"><?=$_SESSION['cry'] .' ' .$util->Forex($util->ApplyDiscount($tprtd))?></span>
                                        </div>
                                    </div>
                                </div>
                                <?php 
